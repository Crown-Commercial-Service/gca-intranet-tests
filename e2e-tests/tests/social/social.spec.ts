...
// // Social features (likes, comments, replies, @mentions) require cross-user interaction,
// // so every test seeds two users (post owner = comment author, post viewer = second actor) and a fresh post.
// // `auth.loginAs(user)` is a placeholder for whatever login-switching helper we land on
// // (separate browser context per user, cookie reset + re-login, etc.) - to be wired up
// // once the scenarios are agreed.
