...
test.describe("homepage - latest news", () => {
  ...
  test("should display article in single column", async ({
    wp,
    homepage,
    runId,
  }) => {
  ...
  test(
    "should display articles in two columns and in order (latest first and oldest last)",
    { tag: "@regression" },
    async ({ wp, homepage, runId }) => {
  ...
  test(
    "should enforce title and paragraph character limits in single column",
    { tag: "@regression" },
    async ({ wp, homepage, runId }) => {
      const post = Post.aPost()
        .withType("news")
  ...
  test(
    "should open news details page",
    { tag: "@regression" },
    async ({ wp, homepage, latestNews }) => {
      const post = Post.aPost()
        .withType("news")
