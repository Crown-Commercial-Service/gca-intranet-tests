...
export default class WpPosts {
  ...
  async clearByRunId(runId: string): Promise<void> {
    ...
    if (wpDriver() === "remote") {
      ...
      await deleteBySearch(`/wp-json/wp/v2/work_updates`);
      await deleteBySearch(`/wp-json/wp/v2/blogs`);
      await deleteBySearch(`/wp-json/wp/v2/news`);
      await deleteBySearch(`/wp-json/wp/v2/media`);
      return;
    }

    // (leave docker/cli branch as-is)
